...
// Yes Bank CSV format — similar to HDFC
// Date,Narration,Chq./Ref.No.,Value Dt,Withdrawal Amt.(Dr.),Deposit Amt.(Cr.),Closing Balance

export function isYesBank(headers: string[]): boolean {
  const h = headers.map((x) => x.toLowerCase().trim());
  return (
    h.some((x) => x.includes("narration")) &&
    (h.some((x) => x.includes("withdrawal amt") || x.includes("withdrawal amt.(dr.)")) &&
...
export function parseYesBank(rows: string[][]): {
